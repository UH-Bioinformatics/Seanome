...
class ProgramRunner():
    commands = {
        "build_lmer_table" : "build_lmer_table -l 12 -sequence %s -freq %s",
        "repeatScout" : "RepeatScout -sequence %s -output %s -freq %s -l 12",
        "bowtie-build" : "bowtie2-build %s %s",
        "bowtie-align" : "bowtie2 -f --score-min L,-0.6,-20  --all -N 1 -L 15    --end-to-end -x %s -U %s -S %s",
        "lastz" : "lastz %s %s --strand=both --identity=%s --format="+LASTZ_OUTPUT_FORMAT+" --masking=1  > %s",
        "nhmmer" : "nhmmer  --cpu 1 %s %s > %s",
        "muscle" : "muscle -in %s  -out %s",
        "addConsensus" : "addConsensus.py %s %s", # tmp needs to be added in the code
        }
    ...
    #     "nhmmer" : PROGS_ROOT_hmmer+"nhmmer  --cpu 1 %s %s > %s",
