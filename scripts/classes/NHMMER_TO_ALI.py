...
class NHMMER_TO_ALI():
    ...
    def __getHitFromHmm__(self, hmmFile):
        ...
        # DLS: modified so that it does not read the entire file into memory
        hitfile = open(os.path.join(self.hmmDir, hmmFile))
        for line in hitfile: 
            line = line.strip()
            if query_length.search(line):
                length = query_length.search(line).groups()[0]
            elif hit_re.search(line):
                # i+=2
                line = hitfile.next()
                line = hitfile.next()
