...
urlpatterns = patterns('seanomeUI.views',
                       ...
                       url(r'^ajax/msa/(?P<jid>\d+)/(?P<ident>outFile_\d+_\d+?)/(?P<clean>[01])/$', 'ajaxGetMSA', name ="ajax_msa"),
                       url(r'^ajax/snp/(?P<jid>\d+)/(?P<ident>outFile_\d+_\d+?)/$', 'ajaxGetSNPs', name ="ajax_snps"),
)
